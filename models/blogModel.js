...
const Blog = {
  ...
  create: ({ users_id, title, content, published_date }, cb) => {
    const sql = 'INSERT INTO blogs (users_id, title, content, published_date) VALUES (?, ?, ?, ?)';
    db.query(sql, [users_id, title, content, published_date], (err, result) => {
      if (err) return cb(err);
